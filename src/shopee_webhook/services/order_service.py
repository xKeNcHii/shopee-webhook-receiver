...
from shopee_webhook.db.models import OrderItem
...
class OrderUpdateInfo(TypedDict, total=False):
    """Information about an order update for Telegram notification."""

    is_new_order: bool
    order_items: List[OrderItem]
    order_data: Dict[str, Any]  # Full order data for new orders


class OrderService:
    """Orchestrates API calls and database operations for orders."""
    ...
    async def process_order_webhook(
        ...
        """
        Process an order webhook by fetching from API and storing in database.
        ...
        Returns:
            OrderUpdateInfo with is_new_order flag and order details/changes, or None on error
        """
        try:
            ...
            order_info = await self.fetch_and_store_order(order_sn)
            return order_info
    ...
    async def fetch_and_store_order(
        self,
        order_sn: str,
    ) -> Optional[OrderUpdateInfo]:
        """
        Fetch order details from API and store in database.

        Args:
            order_sn: Shopee order SN

        Returns:
            OrderUpdateInfo with order details and new items, or None on error
        """
        try:
            ...
            # Convert API response to database insertion format
            items_to_create = self._parse_order_to_items(order_detail)

            if not items_to_create:
                logger.warning(f"No items to create for order {order_sn}")
                return None

            # Format order data for Telegram
            order_data_for_telegram = self._format_order_details(order_detail, items_to_create)

            # Upsert items (delete old, insert new)
            created_items = await self.repository.upsert_order_items(
                order_sn, items_to_create
            )

            logger.info(
                f"Stored {len(created_items)} items for order {order_sn} in database"
            )

            return OrderUpdateInfo(
                is_new_order=True,
                order_items=created_items,
                order_data=order_data_for_telegram,
            )

        except Exception as e:
            logger.error(f"Error fetching/storing order {order_sn}: {e}", exc_info=True)
            return None
    ...
    def _format_order_details(
        ...
        """
        Format full order details for new orders (Telegram notification).
    ...
    def _parse_order_to_items(
        self,
        order: OrderDetailResponse,
    ) -> List[dict]:
        """
        Parse Shopee API order response into list of items for database insertion.
        One row per item.
        ...
        for item in source_items:
            # Determine SKU
            sku = (item.model_sku or item.item_sku or "").strip() if hasattr(item, 'model_sku') else (item.get("model_sku") or item.get("item_sku") or "").strip()
            if not sku:
                item_name = item.item_name if hasattr(item, 'item_name') else item.get('item_name', 'UNKNOWN_ITEM')
